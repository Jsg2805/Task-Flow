...
def create_user():
    ...
    """
    Creates new user.
    Gives in data as JSONs
    Returns:
    201, when user creation is successfull.
    400, Error occurs when an already existing user is being created.
...
def list_users():
    ...
    Returns:
    200,List of all users is displayed
    500, If an =y unexpected error is occured
...
def get_user(user_id):
    try:
        ...
        return jsonify({"error": str(e)}), 500
"""
Gets a specific user when ID is given
Returns,
200,Returns the user 
404,if user doesn't exist
500, unexpted error occurs
